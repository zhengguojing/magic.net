...
namespace Magic.FlowCenter.Service
{
    ...
    public class FlcFlowschemeService : IFlcFlowschemeService, IDynamicApiController, ITransient
    {
        ...
        {
            var entities = await _rep.Context.Queryable<FlcFlowscheme>()
        ...
        public async Task Add(AddFlcFlowschemeInput input)
        {
            ...
            await _rep.Context.Insertable(entity).CallEntityMethod(m => m.Create()).ExecuteReturnEntityAsync();
        }
        ...
        public async Task Delete(DeleteFlcFlowschemeInput input)
        {
            ...
            //假删除
            await _rep.Context.Updateable<FlcFlowscheme>(a => new FlcFlowscheme
            {
        ...
        public async Task<FlcFlowschemeOutput> Get([FromQuery] DeleteFlcFlowschemeInput input)
        {
            ...
            var form = await _rep.Context.Queryable<FlcForm>().InSingleAsync(cachedata.FrmId);
            ...
            if (runtime.nextNodeType != -1 && runtime.nextNode != null && runtime.nextNode.setInfo != null && runtime.nextNodeType != 4)
            {
                ...
                if (temp.NextNodeDesignateType == Setinfo.SPECIAL_USER)
                {
                    ...
                    temp.NextMakerName = string.Join(',', _rep.Context.Queryable<SysUser>().Where(a => temp.NextNodeDesignates.Contains(a.Id.ToString())).Select(a => a.Name).ToList());
                }
                else if (temp.NextNodeDesignateType == Setinfo.SPECIAL_ROLE)
                {
                    ...
                    {
                        var usertemp = _rep.Context.Queryable<SysUser>().LeftJoin<SysUserRole>((a, b) => a.Id == b.SysUserId && b.SysRoleId == long.Parse(item)).Select((a, b) => a).Distinct().ToList();
                    ...
                    temp.NextMakerName = string.Join(',', _rep.Context.Queryable<SysUser>().Where(a => users.Contains(a.Id)).Select(a => a.Name).ToList());
                }
            }
            return temp;
        }
        ...
        public async Task<List<FlcFlowschemeOutput>> List([FromQuery] FlcFlowschemeInput input)
        {
            ...
            var flows = await _rep.Context.Queryable<FlcFlowscheme>()
                                .LeftJoin<FlcForm>((u, b) => u.FrmId == b.Id)
                                       .WhereIF(!string.IsNullOrWhiteSpace(input.OrgId), u => u.OrgId == long.Parse(input.OrgId) || u.OrgId == null || u.OrgId == 0)
