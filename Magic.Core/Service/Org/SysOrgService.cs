...
namespace Magic.Core.Service
{
    ...
    public class SysOrgService : ISysOrgService, IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> QueryOrgPageList([FromQuery] PageOrgInput input)
        {
            ...
            var orgs = await _sysOrgRep.Context.Queryable<SysOrg>()
        ...
        public async Task AddOrg(AddOrgInput input)
        {
            ...
            var newOrg = await _sysOrgRep.Context.Insertable(sysOrg).CallEntityMethod(m => m.Create()).ExecuteReturnEntityAsync();
        ...
        public async Task DeleteOrg(DeleteOrgInput input)
        {
            ...
            try
            {
                ...
                await _sysOrgRep.Context.Deleteable<SysRoleDataScope>(u => childIdList.Contains(u.SysOrgId)).ExecuteCommandAsync();

                // 级联删除该机构子机构对应的用户-数据范围关联信息
                await _sysOrgRep.Context.Deleteable<SysUserDataScope>(u => childIdList.Contains(u.SysOrgId)).ExecuteCommandAsync();
        ...
        public async Task UpdateOrg(UpdateOrgInput input)
        {
            ...
            try
            {
                ...
                await FillPids(sysOrg);
                await _sysOrgRep.Context.Updateable(sysOrg).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Modify()).ExecuteCommandAsync();
                _sysOrgRep.Ado.CommitTran();
        ...
        {
            return await _sysOrgRep.Context.Queryable<SysOrg>().Select(u => u.Id).ToListAsync();
        }
	}
}
