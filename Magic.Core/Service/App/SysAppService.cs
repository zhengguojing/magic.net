...
namespace Magic.Core.Service
{
    ...
    public class SysAppService : ISysAppService, IDynamicApiController, ITransient
    {
        ...
        {
            var apps = await _sysAppRep.Context.Queryable<SysApp>()
        ...
        public async Task AddApp(AddAppInput input)
        {
            ...
            await _sysAppRep.Context.Insertable(app).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
        }
        ...
        public async Task UpdateApp(UpdateAppInput input)
        {
            ...
            var app = input.Adapt<SysApp>();
            await _sysAppRep.Context.Updateable(app).IgnoreColumns(it => new { it.Status }).ExecuteCommandAsync();
        }
