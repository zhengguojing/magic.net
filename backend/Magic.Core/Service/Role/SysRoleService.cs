...
namespace Magic.Core.Service
{
    ...
    public class SysRoleService : ISysRoleService, IDynamicApiController, ITransient
    {
        ...
        {
            return await _sysRoleRep.Context.Queryable<SysRole, SysUserRole>((r, u) => new JoinQueryInfos(JoinType.Inner, r.Id == u.SysRoleId))
        ...
        {
            var roles = await _sysRoleRep.Context.Queryable<SysRole>()
        ...
        {
            return await _sysRoleRep.Context.Queryable<SysRole>()
                                   .WhereIF(!string.IsNullOrWhiteSpace(input.Name), u => u.Name.Contains(input.Name.Trim()))
                                   .WhereIF(!string.IsNullOrWhiteSpace(input.Code), u => u.Code.Contains(input.Code.Trim()))
        ...
        public async Task AddRole(AddRoleInput input)
        {
            ...
            await _sysRoleRep.Context.Insertable(role).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
        }
        ...
        {
            var role = await _sysRoleRep.FirstOrDefaultAsync(u => (u.Name == input.Name || u.Code == input.Code) && u.Id != input.Id);
            if (role.IsNullOrZero())
                throw Oops.Oh(ErrorCode.D1006);
            if (role.RoleType == RoleType.AdminRole)
                throw Oops.Oh("请勿修改管理员角色");
            var sysRole = input.Adapt<SysRole>();
            await _sysRoleRep.Context.Updateable(sysRole).IgnoreColumns(ignoreAllNullColumns:true).CallEntityMethod(m=>m.Modify()).ExecuteCommandAsync();
        }
