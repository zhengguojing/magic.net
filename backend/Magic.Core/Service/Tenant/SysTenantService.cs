...
namespace Magic.Core.Service
{
    ...
    public class SysTenantService : ISysTenantService, IDynamicApiController, ITransient
    {
        ...
        public async Task AddTenant(AddTenantInput input)
        {
            ...
            var newTenant = await _sysTenantRep.Context.Insertable(tenant).CallEntityMethod(m => m.Create()).ExecuteReturnEntityAsync();
        ...
        public async Task InitNewTenant(SysTenant newTenant)
        {
            ...
            newOrg = await _sysOrgRep.Context.Insertable(newOrg).CallEntityMethod(m => m.CreateNotFillTenantInfo()).ExecuteReturnEntityAsync();
            ...
            newRole = await _sysRoleRep.Context.Insertable(newRole).CallEntityMethod(m => m.CreateNotFillTenantInfo()).ExecuteReturnEntityAsync();
            ...
            newUser = await _sysUserRep.Context.Insertable(newUser).CallEntityMethod(m => m.CreateNotFillTenantInfo()).ExecuteReturnEntityAsync();
        ...
        public async Task DeleteTenant(DeleteTenantInput input)
        {
            ...
            List<SysUser> userList = await _sysUserRep.Context.Queryable<SysUser>().Filter(null, true).Where(m => m.TenantId == input.Id).ToListAsync();
            sysTenant.IsDeleted = true;

            try
            {
                _sysUserRep.Ado.BeginTran();
                await _sysTenantRep.Context.Updateable(sysTenant).CallEntityMethod(m => m.Modify()).ExecuteCommandAsync();

                if (userList.Any())
                {
                    userList.ForEach(m => { m.IsDeleted = true; });
                    await _sysUserRep.Context.Updateable(userList).CallEntityMethod(m => m.Modify()).ExecuteCommandAsync();
                }
                _sysUserRep.Context.CommitTran();
                await _sysCacheService.DelByPatternAsync(CommonConst.CACHE_KEY_USERSDATASCOPE);
                await _sysCacheService.DelByPatternAsync(CommonConst.CACHE_KEY_DATASCOPE);
            }
            catch (System.Exception)
            {
                _sysUserRep.Context.RollbackTran();
            }
        ...
        {
            var isExist = await _sysTenantRep.Context.Queryable<SysTenant>().AnyAsync(u => (u.Name == input.Name || u.Host == input.Host || u.Email == input.Email) && u.Id != input.Id && u.TenantType != TenantTypeEnum.SYSTEM);
            if (isExist)
                throw Oops.Oh(ErrorCode.D1300);

            var tenant = input.Adapt<SysTenant>();
            tenant.TenantType = TenantTypeEnum.COMMON;
            await _sysTenantRep.Context.Updateable(tenant).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Modify()).ExecuteCommandAsync();
            await _sysCacheService.DelByPatternAsync(CommonConst.CACHE_KEY_USERSDATASCOPE);
            await _sysCacheService.DelByPatternAsync(CommonConst.CACHE_KEY_DATASCOPE);
        }
        ...
        {
            var sysTenant = await _sysTenantRep.Context.Queryable<SysTenant>().FirstAsync(u => u.Id == input.Id && u.TenantType != TenantTypeEnum.SYSTEM);
            if (sysTenant == null)
        ...
        public async Task<List<OwnMenuOutput>> OwnMenu([FromQuery] QueryTenantInput input)
        {
            ...
            var menuList = await _sysRoleRep.Context.Queryable<SysRoleMenu>().LeftJoin<SysMenu>((t1, t2) => t1.SysMenuId == t2.Id)
        ...
        public async Task ResetUserPwd(QueryTenantInput input)
        {
            ...
            tenantAdminUser.Modify();
            // 更新密码
            await _sysUserRep.Context
                .Updateable<SysUser>(tenantAdminUser)
        ...
        {
            return await _sysUserRep.Context.Queryable<SysUser>().Filter(null, true).FirstAsync(u => u.TenantId == tenantId && u.AdminType == AdminType.Admin);
        }
