...
namespace Magic.Core.Service.Notice
{
    ...
    public class SysNoticeService : ISysNoticeService, IDynamicApiController, ITransient
    {
        ...
        public async Task AddNotice(AddNoticeInput input)
        {
            ...
            var newItem =await _sysNoticeRep.Context.Insertable(notice).CallEntityMethod(m => m.Create()).ExecuteReturnEntityAsync();
        ...
        public async Task UpdateNotice(UpdateNoticeInput input)
        {
            ...
            await _sysNoticeRep.Context.Updateable(notice).CallEntityMethod(m=>m.Modify()).ExecuteCommandAsync();
        ...
        public async Task<dynamic> UnReadNoticeList([FromQuery] NoticeInput input) {
            ...
            var notices = await _sysNoticeRep.Context.Queryable<SysNotice, SysNoticeUser>((n, u) => new JoinQueryInfos(JoinType.Inner, n.Id == u.NoticeId))
             ...
             .ToListAsync();
            var count = await _sysNoticeRep.Context.Queryable<SysNotice, SysNoticeUser>((n, u) => new JoinQueryInfos(JoinType.Inner, n.Id == u.NoticeId)).Where((n, u) => u.UserId == UserManager.UserId && u.ReadStatus == NoticeUserStatus.UNREAD).CountAsync();
